...
public class ComputeMax {
	...
	public static void main(String[] args) {
		...
		// sinon on effecture directement le traitement des données.
			...
			try {
				...
				compute(pathDirectoryLang1,pathDirectoryLang2,langD1S,langD2S,pathDirectoryReport);
	...
	/**
	 * Méthode qui compare les fichiers presents dans les dossiers d1
	 * avec ceux du dossier d2. Les dossier doivent être de langues différentes
	 * 
	 * @param d1
	 *            Chemin vers le premier dossier contenant les fichiers à comparer
	 * @param d2
	 *            Chemin vers le second dossier contenant les fichiers à comparer
	 * @param langD1
	 *            Langue du dossier 1
	 * @param langD2
	 *            Langue du dossier 2
	 ...
	 *            Chemin vers le fichier de rapport
	 */
	public static void compute(String d1, String d2, String langD1, String langD2, String pathReport) {
		// on crée une liste qui contient tout les textes des fichiers du dossier 1 
		...
		if (testComp1.size() < 1 || testComp2.size() < 1) {
			throw new RuntimeException("Vos dossiers doivent contenir au moins un fichier! La comparaison n'aura pas lieu.");
		} else {
			List<List<String>> listOfList = new ArrayList<List<String>>();

			//On récupère les noms de fichier du dossier 1
			List<String> nameFile1 = FileUtil
					.getListNameFile(d1);
			// idem pour le dossier 2
			List<String> nameFile2 = FileUtil
					.getListNameFile(d2);
	...
	/**
	 * Méthode qui récupère la liste de mots associée à chacun des textes contenus dans la liste. 
	 * Elle ajoute en tête de liste le nom du fichier du fichier correspondant et ajoute les listes de mot à la liste de listes entrée en paramètre.
	 * @param list
	 ...
	 */
	private static void computeProcess(List<String> list, List<String> nameList, List<List<String>> listOfList, String lang){
		int i = 0;
		for (String text : list) {
			...
			listBabelId.set(0, nameList.get(i).substring(0, nameList.get(i).length()-4) + "(" + lang + ")");
